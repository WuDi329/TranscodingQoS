...
class KeyFrameExtractor:
    ...
    def rate(self):
        if self._rate is None:
            rate = subprocess.check_output(["mediainfo", "--Inform=Video;%FrameRate%", self.origin_video])
    ...
    def length(self):
        if self._length is None:
            output = subprocess.check_output(["mediainfo", "--Inform=Video;%Duration%", self.origin_video])
    ...
    def _get_middle_time(self, key_time):
        ...
        middle = [round(t + (d/self.rate), 3) for t, d in zip(timestamps[:-1], frameofduration)]
        middle_str = [f"{x:06.3f}" for x in middle]
        output_str = " ".join(middle_str)
        
        with open(os.path.join(self.origin_video.split(".")[0]+"-middletime.txt"), "w") as f:
            f.write(output_str)
        f.close()
    ...
    def _get_key_time(self):
        ...
        command = "ffprobe -skip_frame nokey -select_streams v -show_frames -print_format csv {} | grep -n 'I' | cut -d ',' -f 6 > {}-temp.txt && tr '\n' ' ' < {}-temp.txt >> {}-keytime.txt && rm {}-temp.txt".format(self.origin_video, self.origin_video.split(".")[0], self.origin_video.split(".")[0],  self.origin_video.split(".")[0], self.origin_video.split(".")[0])
